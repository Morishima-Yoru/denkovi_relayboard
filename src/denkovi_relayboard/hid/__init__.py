"""
Adapted from Austin Morton's HID library (MIT License).
Modifications: updated dynamic library search behavior for cross-platform
loading of hidapi shared libraries.

See project LICENSE (MIT) and NOTICE.md for third-party attributions.
"""
...
def __load_cdll() -> ctypes.CDLL:
    # Load in system
    for fname in library_paths:
        with suppress(OSError):
            return ctypes.cdll.LoadLibrary(fname)

    # Load in library locally
    for root, _, files in os.walk(os.path.dirname(__file__)):
        for fname in files:
            if fname not in library_paths:
                continue
            with suppress(OSError):
                return ctypes.cdll.LoadLibrary(os.path.join(root, fname))

    raise ImportError("Unable to load any of the following libraries:{}"\
